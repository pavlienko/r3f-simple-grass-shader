...
export const MainShader = generateShader(
  ...
  void main() {
    ...
    // ROTATE AND POSITIONING(OFFSET) ALL INSTANCES 

    float random = rotation / 6.28;

    float new_x = position.x*cos(rotation ) - position.z*sin(rotation );
    float new_z = position.z*cos(rotation ) + position.x*sin(rotation );
    vec3 Pos = vec3(new_x,position.y,new_z) + offset;

    vec4 modelPosition = modelMatrix * vec4(Pos,1.0);
    modelPosition.y *= max(0.8,random);
    ...
    // 3d noise
    // float displacement = cnoise( Pos * 0.3 + uTime * 0.15)*max(0.,(modelPosition.y + 0.4))* 0.05;

    // sines
    float wave = sin(Pos.x * 2.0 + uTime) * cos(Pos.z * 2.0 + uTime * 0.8);
    float displacement = wave * max(0.0, (modelPosition.y + 0.4)) * 0.1;
  ...
  void main() {
    ...
    vec3 gradient = mix(vec3(0.2,0.1,0.),vec3(0.45,0.53,0.1),height);
    ...
    vec3 finalColor = mix(gradient, fogColor, fogFactor);
